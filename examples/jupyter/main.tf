...
module "hub" {
  ...
  proxy_public_service_host = k8s_networking_k8s_io_v1_ingress.this.spec[0].rules[0].host
    ...
    rules {
      ...
      http {
        paths {
          backend {
            service {
              name = module.proxy.service.metadata[0].name
              port {
                number = module.proxy.service.spec[0].ports[0].port
              }
            }
          }
          path      = "/"
