    ...
    rules {
      ...
      http {
        paths {
          backend {
            service {
              name = module.spiffworkflow-frontend.name
              port {
                number = module.spiffworkflow-frontend.ports[0].port
              }
            }
          }
          path      = "/"
        ...
        paths {
          backend {
            service {
              name = module.spiffworkflow-backend.name
              port {
                number = module.spiffworkflow-backend.ports[0].port
              }
            }
          }
          path      = "/api"
