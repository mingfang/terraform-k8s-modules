
# Module `jitsi/prosody`

Provider Requirements:
* **k8s ([mingfang/k8s](https://registry.terraform.io/providers/mingfang/k8s/latest))** (any version)

## Input Variables
* `ENABLE_AUTH` (default `"1"`)
* `ENABLE_GUESTS` (default `"1"`)
* `JICOFO_AUTH_PASSWORD` (default `"passw0rd"`)
* `JICOFO_AUTH_USER` (default `"focus"`)
* `JICOFO_COMPONENT_SECRET` (default `"s3cr37"`)
* `JIGASI_XMPP_PASSWORD` (default `"passw0rd"`)
* `JIGASI_XMPP_USER` (default `"jigasi"`)
* `JVB_AUTH_PASSWORD` (default `"passw0rd"`)
* `JVB_AUTH_USER` (default `"jvb"`)
* `TZ` (default `"America/New_York"`)
* `XMPP_AUTH_DOMAIN` (default `"auth"`)
* `XMPP_DOMAIN` (default `"meet"`)
* `XMPP_GUEST_DOMAIN` (default `"guest"`)
* `XMPP_INTERNAL_MUC_DOMAIN` (default `"internal-muc"`)
* `XMPP_INTERNAL_MUC_MODULES` (default `""`)
* `XMPP_MODULES` (default `""`)
* `XMPP_MUC_DOMAIN` (default `"muc"`)
* `XMPP_MUC_MODULES` (default `""`)
* `image` (default `"registry.rebelsoft.com/prosody"`)
* `name` (required)
* `namespace` (default `null`)
* `overrides` (default `{}`)
* `ports` (default `[{"name":"5222","port":5222},{"name":"5347","port":5347},{"name":"5280","port":5280}]`)
* `replicas` (default `1`)

## Output Values
* `deployment`
* `name`
* `service`

## Child Modules
* `deployment-service` from `git::https://github.com/mingfang/terraform-k8s-modules.git//archetypes/deployment-service`

