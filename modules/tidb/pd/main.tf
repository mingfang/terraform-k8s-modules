    ...
    containers = [
      {
        ...
        args    = [
          "--name=$(POD_NAME)",
          "--data-dir=/data",
          "--client-urls=http://0.0.0.0:${var.ports[0].port}",
          "--advertise-client-urls=http://$(POD_NAME).${var.name}.${var.namespace}.svc.cluster.local:${var.ports[0].port}",
          "--peer-urls=http://0.0.0.0:${var.ports[1].port}",
          "--advertise-peer-urls=http://$(POD_NAME).${var.name}.${var.namespace}.svc.cluster.local:${var.ports[1].port}",
          "--initial-cluster=${local.initial-cluster}",
          #          "--join=http://${var.name}-0.${var.name}.${var.namespace}.svc.cluster.local:${var.ports[0].port}",
          #          "--L=debug",
        ]
